    ...
    private boolean checkForUpdates() {
        ...
        try {
            ...
            JsonObject latestRelease =
                    jsonArray.get(0).getAsJsonObject(); // Получаем последний релиз
            ...
            isPrerelease =
                    latestRelease
                            .get("prerelease")
                            .getAsBoolean(); // Узнаем, является ли последняя версия предварительной
            // версией
            ...
            for (int i = 0;
                    i < Math.min(currentVersionNumbers.length, latestVersionNumbers.length);
                    i++) {
            ...
            // Если версии major.minor.patch равны и последняя версия является пре-релизной версией,
            // а текущая - нет, то последняя версия не считается более новой
            if (latestVersionParts.length > 1 && currentVersionParts.length == 1) {
                return false;
            }

            // Если текущая версия является пре-релизной, а последняя - нет, то последняя версия
            // считается более новой
            if (currentVersionParts.length > 1 && latestVersionParts.length == 1) {
                return true;
            }

            // Если версии major.minor.patch равны и обе являются предварительными версиями, то
            // считать их равными
            return false;
        } catch (ArrayIndexOutOfBoundsException
                | NumberFormatException
                        misc) { // Если версия начинается с букв или содержит буквы, может выкинуть
            // exception. Пусть на всякий будет.
