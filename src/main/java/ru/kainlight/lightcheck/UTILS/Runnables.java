...
public final class Runnables {
    ...
    public void startBossBarScheduler(CheckedPlayer checkedPlayer) {
        ...
        bossbarScheduler.put(
                checkedPlayer,
                plugin.getServer()
                        .getScheduler()
                        .runTaskTimer(plugin, bossBar::show, 0L, 20L)
                        .getTaskId());
    }
    ...
    private void chatMessage(CheckedPlayer checkedPlayer) {
        ...
        if (checkedPlayer.hasTimer()) {
            ...
            List<String> with_timer =
                    plugin.getMessageConfig().getConfig().getStringList("chat.with-timer");

            with_timer.forEach(
                    message -> {
                        message =
                                message.replace("<inspector>", inspector.getName())
    ...
    private void screenMessages(CheckedPlayer checkedPlayer) {
        ...
        if (timer != null) {
            ...
            if (titleEnabled) {
                String titleMessage =
                        plugin.getMessageConfig().getConfig().getString("screen.check-title");
                String subTitleMessage =
                        plugin.getMessageConfig().getConfig().getString("screen.check-subtitle");
